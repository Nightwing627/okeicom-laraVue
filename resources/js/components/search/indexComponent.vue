        ...
        <div class="l-contentList__list">
          <div class="l-contentList__list__headline l-flex">
            <div class="headlineContent info">
              <h2 class="title" />全てのカテゴリーから検索結果一覧を表示
              <p class="number" />1,000件中 1-20件を表示
            </div>
            ...
            >
              <a class="c-contentList__box__inner">
                ...
                <div class="c-contentList__box__info">
                  ...
                  <div class="number l-flex">
                    <p class="other">
                      <span class="stage" />第{{ lesson.number }}回
                      <span class="date" />{{ moment(lesson.date).format('M/D') }}({{ moment(lesson.date).format('ddd') }}) {{ moment(lesson.start).format('H:mm') }}-{{ moment(lesson.finish).format('H:mm') }}
                    </p>
                    <p class="price" />¥{{ lesson.price.toLocaleString() }}
                  </div>
                  <p class="title" />{{ lesson.title }}
                  <p class="detail pc-only" />{{ lesson.detail }}
                  <div class="category">
                    <span />書道
                    <span />バレエ
                  </div>
...
<script>
  ...
  import VueDatapicker from "vue3-datepicker"
